...
class FedTransformerTrainer(ModelTrainer):
    ...
    def test_on_the_server(self, train_data_local_dict, test_data_local_dict, device, args=None):
        ...
        # total_len = 0
        # for idx, local_test_dl in test_data_local_dict.items():
        #     self.client_trainer.test_dl = local_test_dl
        #     test_sample_len = len(local_test_dl.dataset)
        #     eval_metrics, _, _ = self.client_trainer.eval_model(device=device)
        #     total_len += test_sample_len
        #     for metric_name, metric_value in eval_metrics.items():
        #         if metric_name not in  all_results:
        #             all_results[metric_name] = 0
        #         all_results[metric_name] += metric_value * test_sample_len
        # for metric_name, metric_value in all_results.items():
        #     all_results[metric_name] /= total_len
