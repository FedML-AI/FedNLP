...
def main():
    ...
    parser.add_argument('--beta', type=float, metavar="B", help='beta value for quantity skew')
    ...
    print("minsize of the train data",min([len(i) for i in partition_pkl_train]))
    print("minsize of the test data",min([len(i) for i in partition_pkl_test]))

   
    if('/niid_quantity_clients_%d_beta=%.1f'%(args.client_number,args.beta)in partition):
        del partition['/niid_quantity_clients_%d_beta=%.1f'%(args.client_number,args.beta)]
    if('/niid_quantity_clients=%d_beta=%.1f'%(args.client_number,args.beta)in partition):
        del partition['/niid_quantity=clients_%d_beta=%.1f'%(args.client_number,args.beta)]
    ...
    for i, data in enumerate(partition_pkl):
        ...
        train_path = '/niid_quantity_clients=%d_beta=%.1f'%(args.client_number,args.beta) + '/partition_data/'+str(i)+'/train/'
        test_path = '/niid_quantity_clients=%d_beta=%.1f'%(args.client_number,args.beta) + '/partition_data/'+str(i)+'/test/'
